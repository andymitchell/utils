...
/**
 * Apply a placeholder string and it's propertys to a template string function. 
 * 
 *  
 ...
 * @param params The values, as an array, to replace in the placeholder (i.e. $1, $2)
 * @example if using Drizzle's sql function, which expects sql`SELECT * FROM ${table}`, you can call this instead to achieve the same with applyPlaceholderToTemplateStringFunction(sql, 'SELECT * FROM $1', [table])
 * 
 */
